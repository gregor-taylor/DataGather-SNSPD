...
class DataGatheringapp(Tk):
    def __init__(self, *args, **kwargs):
        ...
        for F in (StartPage, MeasTypePage,ValuesTimePage, WorkingPage, GraphSetupPage, DisplayGraphPage, EDPSetupPage, EfficiencyPage, DCRPage, PlotExistingFile, RTPage):
...
class GraphSetupPage(ttk.Frame):
    ...
    def configure_plot(self, controller, plt_type):
        read_time_and_cols(controller)
...
class DisplayGraphPage(ttk.Frame):
    ...
    def on_show_graph_page(self, controller):
        ...
        if controller.plot_type == "Pwr_Only":
            ...
            sp1_1.plot(controller.plot_arrays_dict[2],controller.plot_arrays_dict[5], 'bo', markersize=2)
            sp1_1.set_ylabel('R (Ohm)')
            sp1_1.set_xlabel('T (K)')
            sp1_1.set_title('R-T')
...
class EfficiencyPage(ttk.Frame):
    ...
    def get_EFF_data(self, controller):
        ...
        if self.bias_id < len(self.biases)+1:    #check doing all points here
            ...
            controller.sim900.write(controller.SIM_slots['VSource'],'OPOFF')    #turn bias off before changing attenuations
            self.atten_id += 1
            controller.measurement_after_id = None
            self.setup_measurement(controller)
...
class DCRPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        graph_button = ttk.Button(self, text="Graph", command=lambda: graph_DCR(controller))
...
class RTPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        bias_r = ttk.Entry(self)
        bias_r.grid(row=2,column=2)

        ttk.Label(self, text="Bias point to run measurement at?:").grid(row=3,column=1)
        bias_point=ttk.Entry(self)
        bias_point.grid(row=3,column=2)
        ...
        start_meas_button.grid(row=4,column=1)

        self.working_label = ttk.Label(self, text="No measurement running", foreground='red')
        self.working_label.grid(row=4,column=2)
        
        stop_meas_button = ttk.Button(self, text="Stop measuring", command=lambda:self.stop_meas(controller))
        stop_meas_button.grid(row=5,column=1)

        graph_button = ttk.Button(self, text="Graph", command=lambda: graph_RT(controller))
        graph_button.grid(row=6,column=1)

        ttk.Button(self, text="Go back to instrument setup page", command=lambda: controller.show_frame(StartPage)).grid(row=7,column=1)
    ...
    def get_RT_data(self, controller):
        ...
        t2 = str(controller.sim900.query(controller.SIM_slots['ThermSlot'],'TVAL? 2')).strip()
        t3 = str(controller.sim900.query(controller.SIM_slots['ThermSlot'],'TVAL? 3')).strip()
        Vsrc=float(controller.sim900.query(controller.SIM_slots['VMeter'],'VOLT? 1,1').strip())
        Vdev=float(controller.sim900.query(controller.SIM_slots['VMeter'],'VOLT? 2,1').strip())
...
def read_time_and_cols(controller):   
