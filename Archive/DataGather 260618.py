...
class DataGatheringapp(Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in (StartPage, MeasTypePage,ValuesTimePage, WorkingPage, GraphSetupPage, DisplayGraphPage, EDPSetupPage, EfficiencyPage, DCRPage):
...
class EDPSetupPage(ttk.Frame):
    def __init__(self, parent, controller):
        Frame.__init__(self, parent)       
        ttk.Label(self, text="Voltage source slot?:").grid(row=1,column=1)
        VSource_slot = ttk.Entry(self)
        VSource_slot.grid(row=1,column=2)
        
        ttk.Button(self, text="System efficiency measurement", command=lambda: self.confirm_Vsrc(controller, VSource_slot.get(), "EfficiencyPage")).grid(row=2,column=1)
        ttk.Button(self, text="Dark counts against bias measurement", command=lambda: self.confirm_Vsrc(controller, VSource_slot.get(), "DCRPage")).grid(row=3,column=1)

        ttk.Button(self, text="Go back to instrument setup page", command=lambda: controller.show_frame(StartPage)).grid(row=4,column=1)
        ttk.Button(self, text="Go back to measurement choice page", command=lambda: controller.show_frame(MeasTypePage)).grid(row=5,column=1)
...
class DCRPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        start_meas_button = ttk.Button(self, text="Start measuring", command=lambda:start_meas(controller, start_bias.get(), stop_bias.get(), bias_step.get(), bias_r.get()))
